...
- Local OpenBao 2.6.1 plugin registration, mount, key creation, signing,
  public-key read, and an `export/myKey` negative probe. The probe returned
  `unsupported path`.
...
| No-export surface | Pass, caveat below | No export/backup/restore route exists; live OpenBao 2.6.1 `export/myKey` returned 404. Key reads and signing expose public material only. [backend.go](../backend.go:41) |
...
| Live `secp256k1/export/myKey` probe | Pass: unsupported path |
