...
"$BAO_BIN" write -format=json -force secp256k1/keys/myKey >"$WORK/create.json"
# Exactly 32 ASCII bytes, deterministic across runs and platforms.
DIGEST_B64="$(printf 'openbao-secp256k1-sec004-test!!!' | base64)"
"$BAO_BIN" write -format=json secp256k1/sign/myKey input="$DIGEST_B64" prehashed=true >"$WORK/sign.json"
...
REQ="$(jq -c 'select(.type=="request" and .request.path=="secp256k1/sign/myKey")' "$AUDIT_LOG" | tail -1)"
RESP="$(jq -c 'select(.type=="response" and .request.path=="secp256k1/sign/myKey")' "$AUDIT_LOG" | tail -1)"
...
KEYREAD="$("$BAO_BIN" read -format=json secp256k1/keys/myKey)"
...
for p in export/myKey backup/myKey restore/myKey; do
